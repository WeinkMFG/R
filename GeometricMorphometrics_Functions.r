...
#Input data set: Morphometric data
#Further Reading:
...
#1.1	Added a line to LMExtract that exports copy of image with landmarks
#1.2	Added the functionality to supply LMExtract with predefined scaling parameter
#1.3	Various little improvements in results presentation of LM.PCA and Hotellingsp
#1.4	Fixed a problem that would break FDM if landmark-distance variance was larger mean landmark-distance
#1.5	Added function PLS.Shape
#1.5.1	Added function PLS.Visual
#1.5.2	Fixed some issues in PLS.Shape and PLS.Visual
#1.5.3	Modified Hotellingsp, so that the proportion of correct classification and the lda model is exported as well
#1.5.4	LM.PCA now allows to override alignment along principal axis by manual alignment for uniform and nonaffine deformation
#1.5.5 	Improved visual output of function Hotellingsp
#1.6	Improved Read.NTS for missing value encoding and adapted NTS export of landmarks to use standard NA-encoding (-999)
#1.6.1	Removed Read.NTS, Read.TPS, and Write.TPS and transferred into separate file MorphoFiles_Function.r
#1.7	Added functionality to work with tif images in LM.extract
#1.8	Added function Allometry
#1.8.1	Corrected LM.PCA function for new rtiff handling of matrix plotting, LM.PCA now returns results
#1.9	Added function LM.PCA.tps
