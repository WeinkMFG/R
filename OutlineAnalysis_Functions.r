...
#Input data set:
#	-Images in the ppm format, possibly binarized
#Further Reading: Claude, J. (2008) "Morphometrics with R". Gentleman, R., Hornik, K., and...
#			Parmigiani, G. (eds) "Use R!", vol. ii, 316 pp. (Springer).
#Disclaimer: Most of the functions in this sheet are at least based on functions developed by...
...
#1.1	Adds option in OutlineExtraction to export the raw (i.e. unsmoothed) outlines as well
#1.2	Fixed a problem that would occur in the success table of the outline extraction, if StartNum was not 1
#1.3	Included function to automatically average over replicates and calculate measurement errors
#2.0	Zahn--Roskies Fourier analysis functions included, fixed a variable naming issue in OutlineAverage
#2.0.1	Power function modified so that results are immediately returned
#2.1	Outline extraction modified, so that a NTS file is exported; Read.NTS function added; NEF function
#		modified, so that uniform orientation can also be achieved according to manually chosen baseline
#2.1.1	Removed Read.NTS and transferred into separate file MorphoFiles_Function.r
#2.2	Removed ImageConversion, Conte, EquiDist, smoothout, and OutlineExtraction to include in separate fucntion file MorphometricExtraction_Functions.r
#--------------------------------------------
#--------------------------------------------
